...
def test_close(setup_block, setup_accounts):
    ...
    with pytest.raises(ValueError):
        setup_block.post(liability_account, 100)
...
def test_block_post(setup_block, setup_accounts):
    ...
    setup_block.post(liability_account, 100)
    setup_block.post(asset_account, 100)
    assert setup_block.count() == 2
    # test post DEBIT to ASSET account
    setup_block.post(asset_account, -10)
    setup_block.post(equity_account, -10)
    assert setup_block.count() == 4
    # test more than two postings
    setup_block.post(asset_account, 1000)
    setup_block.post(liability_account, 100)
    setup_block.post(equity_account, 900)
